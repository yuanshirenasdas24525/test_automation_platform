...
9. **分类 classification**（四选一）：
   ...
   - **环境/其他**：连不上 / 超时 / 5xx / 外部鉴权配置缺失等环境问题。若 token 失效是本报告前序登出、改密、重复登录或参数流转造成的，必须判 **用例问题**，按 7.3/7.4 修复，不能丢给环境。
...
- **`fix.insert_steps`（补可见的前置步骤）**：仅在「用例引用的变量全局无人产出」时给，用于在用例**最前面**插入登录之类的准备步骤。格式：
  `[{"step_name":"管理员登录","config":{"method":"POST","path":"/api/auth/login","params":{"username":"${user_admin}","password":"${password_admin}"},"extract_data":{"admin_token":"$.data.access_token"}}}]`
  硬性约束（违反任意一条平台会整体丢弃）：
  ① 只能用 GET / POST；② 必须 `extract_data` 出至少一个变量，且正是本用例缺的那个；
  ③ **密码等凭据只能引用 `${变量}`（取自可用变量池），绝不能写明文**；④ 最多 3 步。
