...
下列是本模块已被人工评审采纳的用例，**请对齐它们的粒度、步骤写法和风格**（只学格式与粒度，不要照抄内容）：

{{EXEMPLAR_CASES}}

# 避免这些问题（近期被人工拒绝的草稿及原因）
...
以下是本项目已沉淀的业务规则、数据模型、API 契约、术语定义等事实。**用例中出现的字段名、状态值、业务流程、接口行为必须与这里保持一致**；若与你的常识冲突，以这里为准。不要发明这里没有的字段或状态。
