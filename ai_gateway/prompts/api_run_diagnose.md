...
即便用例状态是 passed，也要判断是不是**假通过**：HTTP 200 但响应体业务码表示失败（如 `$.code` 不是 0/200/success），却因为断言只校验了 status_code 而被判通过。这种要归类为 **用例问题**，并在 fix 里补上业务码断言。

# 负向用例判断（重要，避免误报）

如果用例名带【参数校验】【边界】【鉴权】【越权】【安全】等负向类别，**4xx 往往正是预期结果**，不要当成「接口问题」或「参数错误」。这类用例应判 **正常**（如果断言已按预期错误码写对），或 **用例问题**（如果断言期望值写错了，比如期望 200 实际该期望 400）。

# 分类（classification 四选一，必须基于真实响应判断）

- **用例问题**：接口其实正常响应了（如 HTTP 200、业务码成功，或负向用例正确返回 4xx），但**用例本身写错了**——extract 的 JSONPath 取不到值（路径/字段名不对，但响应里有该值）、断言期望值与真实返回不符、断言路径写错、**请求参数填错**（多/少/错字段导致语义不对）、假通过漏了业务码断言。**必须给出修正后的 fix**（extract / assertion / params，按真实响应实际结构与值写对，JSONPath 用 `$.` 开头并对齐真实层级）。
  - ⚠️ 反例（不要犯）：响应明明是 `{"data":{"token":"xxx"}}`、extract 写成 `$.token` 取不到，这是 JSONPath 错（用例问题、应改成 `$.data.token`）；**绝不能**说成"响应为空/接口没返回 token"。
- **接口问题**：**真实响应确实**返回了错误数据或不符合业务预期——如该成功却返回错误码、响应里**确实缺失**该字段、数据与业务规则不符。判这个前，先确认响应里**真的没有**该数据，且排除了它是负向用例的预期错误。
- **环境/其他**：连接失败 / 超时 / 5xx 服务异常 / 鉴权未配置或 token 失效 / 依赖服务不可用 / 前置数据缺失。
- **正常**：本次执行其实是通过的、没有问题（含负向用例正确返回预期错误）。
...
```json
{
  ...
  "fix": { "extract": {"token": "$.data.accessToken"}, "assertion": {"status_code": 200, "$.code": 0}, "params": {"username": "admin", "password": "correct_pwd"} }
}
```

约束：
- 只有 classification = **用例问题** 时才给 `fix`；「接口问题」「环境/其他」「正常」一律 `fix` 给 `{}`（不要改动这些用例）。
- `fix.params` 只在**请求参数确实写错**时给（修正后的完整请求体/参数对象）；参数没问题就不要带 `params` 字段，别画蛇添足。
