...
4. **步骤顺序必须符合真实操作时序**——控件只有在它所在的屏幕/弹框已经打开时才能点：
   - 先**导航到目标屏幕**再操作该屏的控件。例如登录：先点菜单/入口进入登录页，再操作登录表单，不要在表单还没出现时就点它上面的控件。
   - **提交/登录/确认/保存这类按钮排在最后**，先填完所有输入框、再点提交，绝不能把提交按钮排到输入之前。
   - 每一步引用的控件都要属于「当前这一步时刻屏幕上真实可见」的那个页面；跨屏的控件必须先有把那一屏打开的前置点击。
   - **移动 App 常常所有屏幕共用一个 Activity/page_key，元素库里"菜单入口 / 抽屉项 / 目标页表单"会混在同一页——不代表它们同时可见**。必须按真实交互补齐导航链：
     * 打开菜单/抽屉/入口的控件（如汉堡菜单 `open menu`、导航图标）要用 `click` **点开**，不能只 `wait`/`assert_visible` 就当它打开了。
     * 要进某功能页，先 `click` 它的**入口/菜单项**（如抽屉里的 `Log In` 文本项），目标页才会出现，然后再操作该页控件。
     * 典型登录链：`click 汉堡菜单` → `click "Log In" 菜单项` → `input 用户名` → `input 密码` → `click 登录提交按钮` → 断言。不要用 `wait 菜单` 代替 `click 菜单`，也不要跳过"点菜单项进入登录页"这步。
