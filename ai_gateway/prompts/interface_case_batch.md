...
8.2. **前置链用例（仅在本批测试点里出现 `前置链` 类别时才生成；本身是登录/认证接口的模块不要生成前置链）**：准备一个**可复用且可安全改动**的测试账号并登录。
   ...
   - **有创建账号接口（含跨模块的，如 `POST /api/users`）** → 前置链用一条**多步 `requests`**：① **先用 `${my_account}/${my_password}` 登录拿到 `admin_token`**——**创建账号接口几乎都需要管理员权限，下一步建号请求必须带 `Authorization: Bearer ${admin_token}`，漏了会 401**；② 调创建接口建账号（**请求头带 `Authorization: Bearer ${admin_token}`**），用户名 `function:unique(AUTO_TEST_user)`、**密码写固定字面量如 `Test@123`**、**并按接口文档 / digest / 上方「API 约定」补齐该接口声明的其它必填字段（如角色 `role_codes`、类型、状态等）——漏填任何必填字段都会 422**；**关键**：`function:unique` 每次解析都不同，所以**必须从创建响应里 `extract` 出真实用户名**（如 `{"test_user_id": "$.data.id", "test_username": "$.data.username"}`），登录才用得上。③ 用 `${test_username}` + 固定密码 `Test@123` 登录该新账号，`extract` 出**本用例专属的** token（如 `own_token`，按真实层级，如 `{"own_token": "$.data.token"}`）。**绝不能第一步建新号、后面却登录 admin**——那 token 还是 admin 的，没意义。这样产出的是**专属测试账号的 token**，可被改密/删除而不影响系统。
