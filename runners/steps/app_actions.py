...
class AppLaunchStepRunner(BaseStepRunner):
    ...
    def _run(self, step: dict, ctx: ExecutionContext, result: StepResult) -> None:
        session = AppSession.require(ctx)
        config = step.get("config") or {}
        # 允许通过 config 动态 override caps。注意：
        # - driver 还没起来时，update session.caps 会在第一次访问 session.driver 时
        #   通过 _default_driver_factory → _merge_device_caps 走 W3C 规范化；
        # - driver 已经起来了就改不了 caps 了，那时 config 主要影响日志展示。
        if config:
            session.caps.update({k: _resolve_str(v, ctx) for k, v in config.items()})
        # 触发 driver 启动（懒）—— 真正和 Appium 建立 session 就在这一刻
        _ = session.driver
        pkg = config.get("appPackage") or config.get("appium:appPackage") or "(default)"
        result.action = f"launch {pkg}"
