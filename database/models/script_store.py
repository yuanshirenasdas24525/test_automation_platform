...
class ScriptStore(Base):
    ...
    脚本函数统一暴露 handler，外部用 name 作为 function:xxx 或 script 步骤引用。
    workflow 用于 API/Web/App 共用的项目逻辑，并始终在独立脚本进程中执行。
    """
    ...
    requirements = Column(JSONType, nullable=False, default=list, server_default="[]")
