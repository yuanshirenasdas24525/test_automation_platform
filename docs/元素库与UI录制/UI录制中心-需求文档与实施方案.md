...
> 文档版本：v1.5
> 文档状态：实施基线（M0 已完成，M1 Web 真实录制首个切片已完成）
> 编写日期：2026-07-17
> 修订日期：2026-08-07
> 适用项目：Automation Test Platform
> 目标平台：Web / Android / iOS
> 交互参考：[Jam](https://jam.dev/)（参考轻量捕获与上下文交付，不照搬其缺陷录屏数据模型）
> 关联文档：《UI 录制中心-决策记录 ADR》v1.1（本文档所有决策以 ADR 为事实源，见 [`UI录制中心-决策记录-ADR.md`](./UI录制中心-决策记录-ADR.md)）
>
> v1.1 变更摘要：
...
ADR-11 / ADR-12 为 v1.2 对照代码库评审后新增的架构决策；ADR-13 / ADR-14 为 v1.4 技术上下文需求带来的新增决策。**阶段 0 结束前必须修订 ADR-09，并将 ADR-11～14 定版写入独立 ADR 文档；未完成前阶段 1 不得开工。**
