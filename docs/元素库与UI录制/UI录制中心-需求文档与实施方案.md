...
> 1. 曾引入“一次性临时交互”，该方案已由 v1.7 的默认离线交互与显式元素定位模式替代；
...
>
> v1.7 变更摘要（离线交互与元素定位定版）：
> 1. 移除“一次性临时交互”按钮、状态和自动回切逻辑；
> 2. Web 元素库打开页面后自动启动离线回放，默认允许持续点击、输入、弹窗和路由交互；
> 3. 提供 F12 式元素定位开关，开启后悬停高亮并点击锁定元素，且与离线业务交互严格互斥；
> 4. 再次点击定位图标退出元素定位并恢复离线交互。
>
...
13. 离线页面默认可持续交互，并可通过 F12 式定位图标显式切换到非破坏性元素定位；两种模式互斥且状态清晰。
...
### FR-23 默认离线交互、F12 式元素定位与 Web AI 安全探索（v1.7 定版）

默认离线交互与元素定位：

- 进入已完成的 Web 页面资产时自动启动离线回放，默认处于持续交互状态，不要求用户再点击“离线交互”；
- 移除“一次性临时交互”按钮及其临时状态、`Esc` 取消和动作后自动回切逻辑；
- 工具栏提供与浏览器开发者工具相似的元素定位图标，未开启时显示离线交互状态，开启时显示元素定位状态；
- 元素定位开启后必须停止业务点击和输入转发，鼠标悬停时高亮当前候选元素并显示标签、语义名称和尺寸，点击只锁定元素、定位器和右侧详情；
- 再次点击定位图标后退出元素定位并恢复离线交互，两个模式不得同时生效；
- 重叠元素允许在相同位置重复点击循环切换候选，优先精确控件和文字，再退回较大的容器；
- 对离线快照中已失去框架监听的 Portal 弹窗，关闭、取消或 `Esc` 后先等待原处理器，弹窗仍存在时再局部清理弹窗和关联遮罩；
- 所有离线交互仍遵守严格离线策略，不能绕过未命中请求的 `599` 或回源访问原服务。
...
6. AI 输出始终经过白名单和 Schema 校验；系统以当前模块 ID 为硬边界自动筛选适合 UI 自动化的功能用例并检索强相关页面/元素，无合适功能用例时自动降级为“仅元素库”；不提供生成数量参数，候选和生成输出按字符预算自动拆批，输出截断自动拆小重试，上下文裁剪必须显式展示（ADR-06，2026-08-15 修订）；
...
| 6 | AI 生成是否必须关联 functional 用例 | 推荐但非强制；无功能用例时可仅依赖真实元素与录制事实 | ADR-06（2026-08-14 修订） |
...
6. **ADR-06 AI 生成前提**：默认关联 functional 用例；冷启动允许仅元素库模式，两种模式都必须满足生成溯源和事实编译要求。
