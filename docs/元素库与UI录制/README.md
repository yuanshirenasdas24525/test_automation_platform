...
> 整理日期：2026-08-06
> 当前状态：主需求已修订为 v1.5 实施基线；M0 已完成，M1 Web 真实录制首个切片已完成
...
- 定位器必须有快照证据；
- 正式测试仍走现有 v2 Runner；
- Recorder Agent、Web 有头模式、上下文保留和移动端 Native Network 仍需在阶段 0 完成 ADR-11～14 定版；
- ADR-09 需按 v1.4 的 XHR/Fetch 技术上下文范围修订。
...
开始编码前应先更新本目录中的需求基线和 ADR，明确哪些历史结论继续保留、哪些被新交互方案替代。

当前主需求已更新；下一步只需完成 ADR-09 修订、ADR-11～14 定版和阶段 0 技术验证，再进入编码。
