...
> 整理日期：2026-08-11
> 当前状态：主需求已更新为 v1.7；M0～M4 代码闭环、Web 默认离线交互、F12 式元素定位、AI 安全探索与 AI 生成 Web UI 用例首期闭环已实现，Web/Android 已完成真实环境验收，iOS 待宿主机模拟器环境验收
...
### [AI 生成 Web UI 自动化用例实施说明 v1.0](./AI生成Web-UI自动化用例-实施说明-v1.0.md)

2026-08-14 首期实际落地说明，定义并记录：

- AI 以当前模块为硬范围自动筛选功能用例，并只检索模块强相关页面与元素；无合适功能用例时自动降级为仅元素库；
- 不提供人工“生成数量”，候选筛选和用例生成均自动拆批，输出中断时继续拆小重试；
- AI 动作计划 → 元素事实编译 → 待评审草稿 → 正式 Web 用例的闭环；
- 动态值参数化、验证码/滑块人工接管、关键结构断言和可选视觉回归；
- 草稿审计字段、后端白名单、安全门禁及验收方式。
