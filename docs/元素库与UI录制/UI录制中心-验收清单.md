...
2. 选择主基线页面，确认离线回放自动加载并默认可以交互，且加载的是主基线而不是时间最新的录制。
...
10. 默认状态连续点击、输入和切换页面，确认始终执行离线业务动作；点击 F12 式元素定位图标后，悬停元素出现范围和标签提示，点击只选中元素而不执行业务动作；再次点击图标后恢复离线交互。
11. 打开录制时保存的弹窗页面，分别点击右上角关闭、取消和按 `Esc`；确认弹窗与遮罩消失且不会闪动后恢复，底层页面重新可交互或切换到元素定位。

## 4. Web AI 安全探索录制

1. 点击“AI 探索录制”，确认平台创建受控 Web 录制；项目已有主基线时，新会话必须标记为补充录制，不能覆盖主基线。
2. 目标地址是登录页时，确认进度显示“等待人工登录”；在受控浏览器手工登录后探索自动继续，平台不得读取或自动填充密码。
3. 观察进度区，确认显示自动继承的项目已知页面数量，并持续显示当前 URL、已访问页面、发现状态、安全动作、跳过动作及失败动作数量；刷新元素库后仍能恢复当前状态。
...
8. 在探索结果中检查画面、Console、XHR/Fetch、AI 访问/动作事件、URL、浏览器/系统/视口、元素和离线页面，确认与人工 Web 录制使用同一套上下文采集能力。
